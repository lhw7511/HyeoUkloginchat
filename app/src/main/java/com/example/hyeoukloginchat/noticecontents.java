...
private  String id;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        id = getintent.getStringExtra("destinationuids");

        firebaseFirestore.collection("notice").document(id).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    if (task.getResult()!=null){
                        ...
                        Toast.makeText(noticecontents.this, "삭제된 글입니다.", Toast.LENGTH_SHORT).show();
                    }
                }
